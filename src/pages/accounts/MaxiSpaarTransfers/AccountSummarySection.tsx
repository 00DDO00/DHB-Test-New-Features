...
const AccountSummarySection: React.FC<AccountSummarySectionProps> = ({ accountData, accountIban, accountBalance }) => {
  ...
  const balanceNumber = accountBalance ? parseEuroToNumber(accountBalance) : 260000;
  const ratePct = 2.2;
        ...
        <Grid container alignItems="center">
          ...
          <Grid item xs={12} md={4}>
            <Typography variant="h6" sx={{ mb: 1 }}>
              {accountData?.holder_name || 'Loading...'} ⏐ MaxiSpaar
            </Typography>
            <Typography variant="body2" sx={{ opacity: 0.8 }}>
              {accountIban || 'NL24DHBN2018470591'}
            </Typography>
          </Grid>
          ...
          <Grid item xs={12} md={4} sx={{ textAlign: 'center' }}>
            <Typography variant="h4" sx={{ fontWeight: 'bold' }}>
              {accountBalance ? `€ ${parseFloat(accountBalance).toLocaleString('nl-NL', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` : '€ 260.000,00'}
            </Typography>
          </Grid>
